...
export default class SidebarMobile extends Component {
  ...
  _getSections() {
    ...
    return this.props.sections.map(section => {
      const active = pathname === section.url || pathname.includes(`/${section.url}`);
      const absoluteUrl = `/${section.url}`;
  ...
  _handleTouchMove(e){
    const xDiff = initialTouchPosition.x - e.touches[0].pageX;
    const yDiff = initialTouchPosition.y - e.touches[0].pageY;
  ...
  _handleOpenerTouchMove(e){
    const xDiff = e.touches[0].pageX - initialTouchPosition.x;
    const yDiff = initialTouchPosition.y - e.touches[0].pageY;
    const factor = Math.abs(yDiff / xDiff);
